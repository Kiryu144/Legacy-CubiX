...
#define ENET_RANGE_CODER_FREE_SYMBOLS                                                              \
	{                                                                                              \
		if( nextSymbol                                                                             \
			>= sizeof( rangeCoder->symbols ) / sizeof( ENetSymbol ) - ENET_SUBCONTEXT_ORDER )      \
		{                                                                                          \
			nextSymbol = 0;                                                                        \
...
#ifdef ENET_CONTEXT_EXCLUSION
		if( childContext->total > ENET_SUBCONTEXT_SYMBOL_DELTA + ENET_SUBCONTEXT_ESCAPE_DELTA )
			ENET_CONTEXT_ENCODE_EXCLUDE(
				childContext, value, under, total, ENET_CONTEXT_SYMBOL_MINIMUM );
#endif
...
#define ENET_CONTEXT_TRY_DECODE(                                               \
	context, symbol_, code, value_, under_, count_, update, minimum, exclude ) \
	ENET_CONTEXT_DECODE( context,                                              \
						 symbol_,                                              \
						 code,                                                 \
						 value_,                                               \
						 under_,                                               \
...
#define ENET_CONTEXT_ROOT_DECODE(                                               \
	context, symbol_, code, value_, under_, count_, update, minimum, exclude )  \
	ENET_CONTEXT_DECODE(                                                        \
		context,                                                                \
		symbol_,                                                                \
	...
	void* context, const enet_uint8* inData, size_t inLimit, enet_uint8* outData, size_t outLimit )
{
	...
	for( ;; )
	{
			 ...
			 subcontext = &rangeCoder->symbols[ subcontext->parent ] )
		{
			...
			if( childContext->total > 0 )
			{
				ENET_CONTEXT_TRY_DECODE( subcontext,
										 symbol,
										 code,
										 value,
										 under,
