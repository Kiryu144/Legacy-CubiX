...
static void enet_protocol_notify_connect( ENetHost* host, ENetPeer* peer, ENetEvent* event )
{
	...
	if( event != NULL )
	{
		...
		enet_protocol_dispatch_state( host,
									  peer,
											   ...
											   enet_uint8** currentData )
{
	...
	*currentData += dataLength;
	...
	if( enet_peer_queue_incoming_command( peer,
										  command,
...
static int enet_protocol_handle_incoming_commands( ENetHost* host, ENetEvent* event )
{
	...
	sessionID
		= ( peerID & ENET_PROTOCOL_HEADER_SESSION_MASK ) >> ENET_PROTOCOL_HEADER_SESSION_SHIFT;
	...
	while( host->continueSending )
			 ...
			 ++currentPeer )
		{
			...
			sentLength = enet_socket_send(
				host->socket, &currentPeer->address, host->buffers, host->bufferCount );
...
int enet_host_service( ENetHost* host, ENetEvent* event, enet_uint32 timeout )
{
	...
	do
	{
		...
		do
		{
			...
			if( enet_socket_wait( host->socket,
								  &waitCondition,
