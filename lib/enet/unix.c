...
int enet_address_get_host( const ENetAddress* address, char* name, size_t nameLength )
{
	...
	err = getnameinfo(
		( struct sockaddr* )&sin, sizeof( sin ), name, nameLength, NULL, 0, NI_NAMEREQD );
	if( !err )
	{
	...
	gethostbyaddr_r( ( char* )&in,
					 ...
					 &errnum );
#else
	hostEntry = gethostbyaddr_r( ( char* )&in,
								 sizeof( struct in_addr ),
								 AF_INET,
								 &hostData,
								 buffer,
...
int enet_socket_set_option( ENetSocket socket, ENetSocketOption option, int value )
{
	...
	case ENET_SOCKOPT_NONBLOCK:
		...
		result = fcntl( socket,
						F_SETFL,
						( value ? O_NONBLOCK : 0 ) | ( fcntl( socket, F_GETFL ) & ~O_NONBLOCK ) );
#else
	...
	case ENET_SOCKOPT_RCVTIMEO:
	{
		...
		result			= setsockopt(
			 socket, SOL_SOCKET, SO_RCVTIMEO, ( char* )&timeVal, sizeof( struct timeval ) );
		break;
	}
	...
	case ENET_SOCKOPT_SNDTIMEO:
	{
		...
		timeVal.tv_usec = ( value % 1000 ) * 1000;
		result			= setsockopt(
			 socket, SOL_SOCKET, SO_SNDTIMEO, ( char* )&timeVal, sizeof( struct timeval ) );
		break;
	}
